...
export function ProfilePhotoField({ file, currentUrl, error, required = false, onChange, onRemove }: ProfilePhotoFieldProps) {
  ...
  return <fieldset className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-background)] p-4" aria-describedby={error ? 'profile-photo-error' : undefined}>
    ...
    <div className="flex flex-wrap items-center gap-4">
      {previewUrl ? <img alt={t('profilePhoto.previewAlt')} className="size-24 rounded-2xl object-cover shadow-sm" src={previewUrl} /> : <div aria-hidden="true" className="grid size-24 place-items-center rounded-2xl bg-[var(--color-primary-surface)] text-3xl font-black text-[var(--color-primary)]">+</div>}
    ...
    <p className="mt-3 text-sm text-[var(--color-text-secondary)]">{t('profilePhoto.help')}</p>

...
export function AppointmentConfirmationPage() {
  ...
  if (!draft) return <PatientLayout activeSection="book"><ErrorState contained message={t('patient.appointmentConfirmation.missing')} onRetry={() => navigate('/patient/doctors/search')} retryLabel={t('patient.appointmentConfirmation.findDoctor')} /></PatientLayout>
  if (state === 'loading') return <PatientLayout activeSection="book"><LoadingState contained message={t('patient.appointmentConfirmation.loading')} /></PatientLayout>
  if (state === 'error') return <PatientLayout activeSection="book"><ErrorState contained message={t('patient.appointmentConfirmation.error')} onRetry={() => { setState('loading'); setAttempt((value) => value + 1) }} retryLabel={t('patient.home.retry')} /></PatientLayout>
  if (state === 'unavailable' || !doctor || !slot) return <PatientLayout activeSection="book"><ErrorState contained message={message || t('patient.appointmentConfirmation.slotUnavailable')} onRetry={() => navigate(buildDoctorBookingPath(draft.doctorId))} retryLabel={t('patient.appointmentConfirmation.chooseAgain')} /></PatientLayout>

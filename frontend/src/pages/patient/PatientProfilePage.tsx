...
export function PatientProfilePage() {
  ...
  return <PatientLayout activeSection="profile" isLoggingOut={false} onAppointments={() => undefined} onDashboard={() => navigate('/patient/home')} onLogout={signOut} onProfile={() => undefined}><div className="mx-auto max-w-3xl px-5 py-8"><section className="rounded-3xl border border-[var(--color-border)] bg-white p-6 shadow-sm sm:p-8"><h1 className="text-3xl font-extrabold">{t('profilePage.patientTitle')}</h1><p className="mt-2 text-[var(--color-text-secondary)]">{t('profilePage.patientDescription')}</p>{user && <div className="mt-6"><ProfilePhotoField currentUrl={user.profile_image_url} file={file} onChange={setFile} onRemove={remove} /><button className="mt-5 rounded-full bg-[var(--color-primary)] px-6 py-3 font-bold text-white disabled:opacity-50" disabled={!file || busy} onClick={save}>{t(busy ? 'profilePage.saving' : 'profilePage.save')}</button></div>}<p aria-live="polite" className="mt-4 min-h-6 text-sm font-semibold text-[var(--color-primary)]">{message ? t(message) : ''}</p></section></div></PatientLayout>
}

...
import { getDoctorAppointments, type DoctorAppointment } from '../../services/doctorAppointmentService'
...
export function DoctorAppointmentsPage() {
  ...
  const today = sudanDateKey(new Date()); const todayAppointments = appointments.filter((item) => sudanDateKey(new Date(item.starts_at)) === today); const upcoming = appointments.filter((item) => sudanDateKey(new Date(item.starts_at)) > today); const previous = appointments.filter((item) => sudanDateKey(new Date(item.starts_at)) < today)
  const section = (key: 'today' | 'upcoming' | 'previous', items: DoctorAppointment[]) => <section className="mt-7" aria-labelledby={`appointments-${key}`}><div className="flex items-end justify-between gap-3"><div><h2 className="text-2xl font-extrabold" id={`appointments-${key}`}>{t(`doctor.appointments.sections.${key}`)}</h2><p className="mt-1 text-sm text-[var(--color-text-secondary)]">{t(`doctor.appointments.sectionDescriptions.${key}`)}</p></div><span className="rounded-full bg-[var(--color-primary-surface)] px-3 py-1 text-sm font-bold text-[var(--color-primary)]">{items.length}</span></div>{items.length ? <div className="mt-4 grid gap-4 lg:grid-cols-2">{items.map((item, index) => <div key={item.id} style={{ animationDelay: `${index * 55}ms` }}><AppointmentCard appointment={item} /></div>)}</div> : <div className="mt-4 rounded-3xl border border-dashed border-[var(--color-border)] bg-white p-7 text-center text-[var(--color-text-secondary)]">{t(`doctor.appointments.empty.${key}`)}</div>}</section>

  return <DoctorLayout activeSection="appointments" isLoggingOut={loggingOut} onAppointments={() => undefined} onAvailability={() => navigate('/doctor/availability')} onLogout={signOut} onProfile={() => navigate('/doctor/profile')}><div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:px-10"><header className="rounded-3xl border border-teal-100 bg-gradient-to-r from-[var(--color-primary-surface)] to-white p-6 rtl:bg-gradient-to-l sm:p-8"><p className="font-bold text-[var(--color-primary)]">{t('doctor.appointments.eyebrow')}</p><h1 className="mt-1 text-3xl font-extrabold sm:text-4xl">{t('doctor.appointments.title')}</h1><p className="mt-2 max-w-3xl text-[var(--color-text-secondary)]">{t('doctor.appointments.description')}</p></header>{state === 'loading' ? <LoadingState contained message={t('doctor.appointments.loading')} /> : state === 'error' ? <ErrorState contained message={t('doctor.appointments.error')} onRetry={() => { setState('loading'); setAttempt((value) => value + 1) }} retryLabel={t('doctor.dashboard.retry')} /> : appointments.length === 0 ? <div className="mt-7 rounded-3xl border border-[var(--color-border)] bg-white p-10 text-center shadow-sm"><h2 className="text-xl font-extrabold">{t('doctor.appointments.emptyTitle')}</h2><p className="mt-2 text-[var(--color-text-secondary)]">{t('doctor.appointments.emptyDescription')}</p></div> : <>{section('today', todayAppointments)}{section('upcoming', upcoming)}{section('previous', previous)}</>}</div></DoctorLayout>
}
